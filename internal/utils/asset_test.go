...
func TestUploadAssetsUtils(t *testing.T) {
	...
	defer func() { _ = os.RemoveAll(filepath.Join("uploads", folder)) }()

	// Check file exists
	if _, err := os.Stat(savedPath); os.IsNotExist(err) {
		t.Fatalf("uploaded file does not exist at path: %s", savedPath)
	}

	// Check file content
	uploadedContent, err := os.ReadFile(savedPath)
	if err != nil {
...
func TestDeleteAssetsUtils(t *testing.T) {
	// Prepare dummy file content
	fileContent := []byte("delete test file content")
	tempFile, err := os.CreateTemp("", "delete-upload-*.txt")
	if err != nil {
		t.Fatalf("failed to create temp file: %v", err)
	}
	defer func() { _ = os.Remove(tempFile.Name()) }()
	...
	// Reopen for multipart simulation
	srcFile, err := os.Open(tempFile.Name())
	if err != nil {
		t.Fatalf("failed to reopen temp file: %v", err)
	}
	...
	// Clean up
	_ = os.RemoveAll(filepath.Join("uploads", folder))
}
