...
import {type Control, useFieldArray, useForm, useFormState, useWatch} from "react-hook-form";
...
export function ProductActionEdit({product, categories, units}: ProductActionEditProps) {
  ...
  const reset: (product?: Product | null) => void = () => {
    ...
    form.reset({
      ...
      variant_groups: Object.entries(grouped).map(([type, variants]) => ({
        type,
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <div className="mx-4 grid grid-cols-2 gap-12 p-8">
                ...
                render={({ field }) => (
                  <FormItem className="w-full">
                    ...
                    <FormControl>
                      <Select
                        value={field.value ? String(field.value) : (
                          form.getValues("category_id") === product?.category_id
      ...
      <AccordionContent>
        ...
        <div className="flex justify-between my-4">
          <Button
            ...
            className="cursor-pointer"
            onClick={() => {
              // Compute new negative ID for temporary variant
